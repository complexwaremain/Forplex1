...
Nebula = function()
	...
	GameThemeMode = GameTheme.CreateDropdown({
		Name = "Theme",
		Function = function() end,
		List = {"Old", "Winter", "Halloween", "Nebula"}
	})
end)
		...
		Function = function(callback)
			if callback then 
				...
				if plrgui:FindFirstChild("KeyStrokes") then 
					...
					warningNotification("KeyStrokes Error", "Couldn't find KeyStrokes in playergui, please dm Complex about this\nOr if you know what you're doing, you can enable the debugs in the code and try fix them.", 60)
				end
			end
		end
	})
...
local function handleEntityDeathEvent(deathTable, lplr, DeathMessage, FinalKillMessage)
    ...
    if deathTable.finalKill then
        if killed == lplr and killer ~= lplr then
            local message = getNotificationMessage({}, "You have lost to <name>. Forplex.", "<name>", killer.DisplayName)
    ...
    run(function()
        ...
        local function checkPlayerList(player)
            for _, playerToCheck in ipairs(PlayersToCheck) do
                local nameToDisplay = DisplayNameEnabled and playerToCheck.DisplayName or playerToCheck.Username
                if player.Name == playerToCheck.Username or player.DisplayName == playerToCheck.DisplayName then
                    warningNotification("Forplex", nameToDisplay .. " has joined", 5)
                    if LeaveEnabled then
                        wait(LeaveDelay) 
                        game:Shutdown()  
                    end
